...
write.table(resgenome, file = "/gpfs/hpc/home/liiskolb/transqtl_final/article_figures/suppl_data/all_crediblesets.tsv", sep = "\t", quote = F)
...
## results are filtered in the get_trans_for_meta.R script using the resgenome dataframe

# Read the results after trans-filtering
...
get_metacs_summary = function(metacs){
  ...
  metacs_summary = data.frame("metacs_lead" = names(metacs[["meta_cs"]]), "nr_snps" = unlist(lapply(metacs[["meta_snps"]], length)), "nr_cs(nr_clusters)" = unlist(lapply(metacs[["meta_cs"]], length)))
...
get_short_link <- function(query, organism = "hsapiens", multi_query = FALSE, filter = "default", ordered_query = FALSE, significant = TRUE, 
  ...
  body <- jsonlite::toJSON((
    list(
      url = jsonlite::unbox("https://biit.cs.ut.ee/gprofiler_beta/gost"),
...
res_filt = resgenome %>% filter(nr_trans_in_cl > 0)
resfdr_filt = res_filt %>% filter(lead_FDR < 0.1) # with FDR 10%
resbonf_filt = res_filt %>% filter(lead_pval < pval_thr) # with bonferroni threshold

metares_filt = get_metacrediblesets(res_filt, thr = 0) # Genome-wide thr
...
metafdr_filt = get_metacrediblesets(resfdr_filt, thr = 0) # FDR thr
...
# merge clusters by meta_ids
metalist_filt <- split(as.character(res_filt$cl_id2), as.character(res_filt$meta_id))
...
metares_summary_filt2 = metares_summary_filt %>% rowwise() %>% mutate(nr_trans_in_cl = paste0(unique(res_filt[res_filt$meta_id == metacs_lead,c("meta_id", "cl_id2", "qtl_group", "nr_trans_in_cl")])[["nr_trans_in_cl"]], collapse = ";")) %>% data.frame()

## Add info if snp remains after FDR and after strict bonferroni
...
approach_summary = res_filt %>% select(approach, meta_id) %>% distinct() %>% group_by(meta_id) %>% summarise(separate = "separate" %in% approach, integrated = "integrated" %in% approach)
...
cl_method_summary = res_filt %>% select(cl_method, meta_id) %>% distinct() %>% group_by(meta_id) %>% spread(cl_method, 1) %>% data.frame()
