...
#parser$add_argument("-g", "--geno", help="Genotype file (tsv)") 
parser$add_argument("-d", "--gds", help="Genotype file (gds)") 
parser$add_argument("-c", "--cov", action="store_true", default=TRUE, help="Add if should include covariates")
parser$add_argument("-r", "--resdir", help="Directory to save the results to")
...
if (!is.null(args$expr) & !is.null(args$gds)) {
  ...
  print(paste("Genotypes are in:", args$gds))
...
eQTL_wrapper <- function(gds_file, exprs_file, cov_data = TRUE, resdir){
  # Get genotypes data
  genotypes = gdsToMatrix(gds_file)
  gen_data = genotypes$genotypes # Matrix with full genotype data (id-s in columns)
  #gen_data = fread(gen_file)
  #gen_data = data.frame(gen_data)
  #gen_data = gen_data[!duplicated(gen_data$V1),]
  #row.names(gen_data) = gen_data$V1
  #gen_data$V1 = NULL
  ...
  exprs_data = read.table(exprs_file, sep="\t", header = TRUE, stringsAsFactors = F, check.names = F) 
  # remember sample_id
  ...
  sample_ids = intersect(colnames(exprs), colnames(gen_data)) # genotype ids actually
  
  if(length(sample_ids)==0){
    stop("Genotype ids don't match at all")
  }else{
  ...
  me = runMatrixEQTL(exprs, as.matrix(gens), as.matrix(covs), pvOutputThreshold = 1, model = modelLINEAR, resfile = NULL)
  print("qtl done")
  # Save file as gzip
...
# Run the analysis
#res = eQTL_wrapper(gen_file = args$geno, gds_file = args$gds, exprs_file = args$expr, cov_data = args$cov, resdir = args$resdir)
res = eQTL_wrapper(gds_file = args$gds, exprs_file = args$expr, cov_data = args$cov, resdir = args$resdir)
